...
def derive_back(species):
    grid, _ = daemon_generator.SPECIES[species]
    cfg = CONFIG[species]
    out = list(grid)
    for y in range(H):
        row = out[y]
...
def derive_view(species, direction):
    ...
    front, _ = daemon_generator.SPECIES[species]
    cfg = CONFIG[species]
    if direction == "down":
        return front
    if direction == "up":
...
def make_daemon_facing(species, direction, variant=None, rng=random):
    _, ramps = daemon_generator.SPECIES[species]
...
def all_species_sheet(scale=5):
    pad = 6
    cell = W + pad
    rows = len(daemon_generator.SPECIES)
    sheet = Image.new("RGBA", (8 * cell + pad, rows * cell + pad),
                      (58, 58, 70, 255))
    for r, name in enumerate(daemon_generator.SPECIES):
