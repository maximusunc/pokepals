...
# array would crash to_vec2(it["position"]) — so the group notes live in _interactables_comment).
parts = [",\n".join(line(e) for e in group) for _note, group in interactable_groups if group]
body = ",\n\n".join(parts)
INTER_COMMENT = "Everything you can walk up to and Examine, grouped in array order: " + "; ".join(
    note for note, group in interactable_groups if group) + ". Stalls/crates are generated by tools/gen_bazaar.py — edit that and re-run to re-pack, don't hand-tune positions."
...
out.append('\t],')
out.append('\t"_interactables_comment": %s,' % json.dumps(INTER_COMMENT))
out.append('\t"interactables": [')
out.append(body)
out.append('\t],')
...
d = json.loads(text)
ids = [i["id"] for i in d["interactables"] if i.get("id")]
assert len(ids) == len(set(ids)), "dup ids"
assert any(i["type"] == "shopkeeper" for i in d["interactables"]), "no shopkeeper"
assert any(p["id"] == "bazaar_entry" for p in d["portals"]), "no portal"
assert d["npc_companion"], "no npc"
print("PARSES OK | interactables(incl _group sentinels):", len(d["interactables"]), "| real ids:", len(ids))
