defmodule Server.PresenceFramesTest do
  ...
  `Server.WorldChannel` pushes verbatim. Ids are the players' `user_id`s (the Presence keys).
  ...
  # Stand-in user_ids (real ones are UUIDs; any stable string works for the pure logic).
  @me "user-me"
  @u2 "user-2"
  @u3 "user-3"
  ...
  test "a fresh peer with no identity yet becomes a bare join" do
    {frames, known} =
      PresenceFrames.diff_to_frames(@me, MapSet.new(), %{@u2 => meta(%{})}, %{})

    assert frames == [{"join", %{id: @u2}}]
    assert MapSet.member?(known, @u2)
  end

  test "a fresh peer that already has identity becomes join + identity (in that order)" do
    {frames, known} =
      PresenceFrames.diff_to_frames(@me, MapSet.new(), %{@u3 => meta(%{"name" => "Mossfen"})}, %{})

    assert frames == [{"join", %{id: @u3}}, {"identity", %{"name" => "Mossfen", id: @u3}}]
    assert MapSet.member?(known, @u3)
  end
  ...
  test "an identity update (key in BOTH joins and leaves) is identity-only, never leave+rejoin" do
    {frames, known} =
      PresenceFrames.diff_to_frames(
        @me,
        MapSet.new([@u2]),
        %{@u2 => meta(%{"name" => "Mossfen"})},
        %{@u2 => meta(%{})}
      )

    assert frames == [{"identity", %{"name" => "Mossfen", id: @u2}}]
    assert MapSet.member?(known, @u2), "the peer stays known across an update"
  end

  test "a real leave emits a leave frame and drops the peer from known" do
    {frames, known} =
      PresenceFrames.diff_to_frames(@me, MapSet.new([@u2]), %{}, %{@u2 => meta(%{})})

    assert frames == [{"leave", %{id: @u2}}]
    refute MapSet.member?(known, @u2)
  end

  test "leaving an unknown peer is a no-op (never seen by this client)" do
    {frames, known} =
      PresenceFrames.diff_to_frames(@me, MapSet.new(), %{}, %{"user-9" => meta(%{})})
  ...
  test "our own id is ignored in every branch" do
    # our own join...
    {frames, known} =
      PresenceFrames.diff_to_frames(@me, MapSet.new(), %{@me => meta(%{"name" => "me"})}, %{})

    assert frames == []
    refute MapSet.member?(known, @me)
  ...
  test "welcome/diff race: a join for an already-known peer sends identity, not a second join" do
    # Peer @u2 was already in `welcome` (so already known); a join diff for it now must NOT re-announce
    # the join (which would double-spawn its puppet) — only its identity, if present.
    {frames, known} =
      PresenceFrames.diff_to_frames(@me, MapSet.new([@u2]), %{@u2 => meta(%{"name" => "X"})}, %{})

    assert frames == [{"identity", %{"name" => "X", id: @u2}}]
    assert MapSet.member?(known, @u2)
  ...
  test "peers/2 shapes a presence list into the welcome roster, excluding us" do
    ...
    peers = PresenceFrames.peers(presence_list, @me) |> Enum.sort_by(& &1.id)

    assert peers == [
             %{id: @u2, identity: %{"name" => "Mossfen"}},
             %{id: @u3, identity: %{}}
           ]
  end
end
